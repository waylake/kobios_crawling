...
with ThreadPoolExecutor(max_workers=10) as executor:
    ...
    for future in tqdm(as_completed(tasks), total=len(tasks)):
        try:
            ...
            for period_data in ratios["seat"]:
                results_list.append({
                    '영화명': period_data['movie_name'],
                    '기간': period_data['period'],
                    '주차': period_data['week'],
                    '좌석점유율': period_data['seat_cnt_ratio'],
                    '좌석점유율-순위': period_data['rank'],
                    '좌석점유율-개봉일': period_data['open_date'],
                    '좌석점유율-판매비율': period_data['sale_cnt_ratio'],
                    '좌석점유율-좌석수': period_data['seat_cnt'],
                    '좌석점유율-총액': period_data['show_amt'],
                    '좌석점유율-추가총액': period_data['add_show_amt'],
                    '좌석점유율-횟수': period_data['show_cnt'],
                    '좌석점유율-추가횟수': period_data['add_show_cnt'],
                    'target_movie': target_movie
                })

            for period_data in ratios["show"]:
                for item in results_list:
                    if item['영화명'] == period_data['movie_name'] and item['기간'] == period_data['period']:
                        item.update({
                            '상영점유율': period_data['dsp_cnt_ratio'],
                            '상영점유율-순위': period_data['rank'],
                            '상영점유율-개봉일': period_data['open_date'],
                            '상영점유율-상영횟수': period_data['dsp_cnt'],
                            '상영점유율-상영비율': period_data['dsp_cnt_ratio'],
                            '상영점유율-발행액': period_data['issu_amt'],
                            '상영점유율-추가발행액': period_data['add_issu_amt'],
                            '상영점유율-발행수': period_data['issu_cnt'],
                            '상영점유율-추가발행수': period_data['add_issu_cnt']
                        })

            for period_data in ratios["screen"]:
                for item in results_list:
                    if item['영화명'] == period_data['movie_name'] and item['기간'] == period_data['period']:
                        item.update({
                            '스크린점유율': period_data['scrn_cnt_ratio'],
                            '스크린점유율-순위': period_data['rank'],
                            '스크린점유율-개봉일': period_data['open_date'],
                            '스크린점유율-스크린수': period_data['scrn_cnt'],
                            '스크린점유율-스크린비율': period_data['scrn_cnt_ratio'],
                            '스크린점유율-발행액': period_data['issu_amt'],
                            '스크린점유율-추가발행액': period_data['add_issu_amt'],
                            '스크린점유율-발행수': period_data['issu_cnt'],
                            '스크린점유율-추가발행수': period_data['add_issu_cnt']
                        })
